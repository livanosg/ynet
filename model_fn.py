...
def ynet_model_fn(features, labels, mode, params):
    if params['no_distribution']:
        device_0 = tf.device('/CPU:0')
        device_1 = tf.device('/GPU:0')
        device_2 = tf.device('/GPU:1')
    else:
        device_0 = device_1 = device_2 = nullcontext()
        ...
        with tf.name_scope('Branch_{}_training'.format(params['branch'])):
            with tf.name_scope('{}'.format(mode)):  # The Inputs and outputs of the algorithm
                with device_0:
                    ...
                    summary.image('2_Output_1_label'.format(params['branch']),
                                  tf.expand_dims(tf.cast(label_1 * 255, dtype=tf.uint8), axis=-1), max_outputs=1)
                    summary.image('3_Output_1', tf.expand_dims(tf.cast(output_1_arg * 255, dtype=tf.uint8), axis=-1),
                                  max_outputs=1)
                    summary.image('4_Final', tf.expand_dims(tf.cast(final_output * 255, dtype=tf.uint8), axis=-1),
                                  max_outputs=1)
                    summary.image('5_Output_2',
                                  tf.expand_dims(tf.cast(output_2_arg * 127 + 1, dtype=tf.uint8), axis=-1),
                                  max_outputs=1)
                    summary.image('6_Output_2_label'.format(params['branch']),
                                  tf.expand_dims(tf.cast(label_2, dtype=tf.uint8) * 127 + 1, axis=-1), max_outputs=1)
